...
// استيراد المكتبات مع التأكد من تثبيتها في المشروع
...
const App: React.FC = () => {
  ...
  const handleChatSend = () => {
    ...
    if (!isClean) {
      setChatMessages([...chatMessages, { id: Date.now(), text: "تنبيه من البوت المراقب: تم حجب الرسالة لمخالفتها ميثاق الالتزام المهني والوقار القانوني", sender: 'bot' }]);
    } else {
  ...
  const renderContent = () => {
    switch (currentSection) {
      case 'استشارة قانونية':
        return (
          <div style={{ animation: 'fadeIn 0.5s' }}>
            <div style={{ background: 'rgba(251, 191, 36, 0.1)', border: '1px solid #fbbf24', color: '#fbbf24', padding: '1.5rem', borderRadius: '1rem', marginBottom: '1.5rem', textAlign: 'right' }}>
              <h4 style={{ margin: '0 0 10px 0' }}>💡 لمحة تعريفية بالمنصة:</h4>
              <p style={{ margin: '0 0 10px 0' }}>تعد هذه المنصة بيئة رقمية مخصصة لـ <strong>الأغراض التعليمية والتدريبية</strong> فقط. تهدف إلى تمكين المستخدمين وتزويدهم بالمعرفة اللازمة في إطار تدريبي ومنهجي مدروس.</p>
              <p style={{ margin: 0, color: '#ff4d4d', fontWeight: 'bold' }}>⚠️ تنبيه: إستخدام معلومات هذه منصة لا يغني عن إستشارة محامي او الجهات المختصة.</p>
            </div>
        ...
        if (!agreedToCharter) {
          return (
            <div style={styles.charterBox}>
              ...
              <button onClick={() => setAgreedToCharter(true)} style={{ width: '100%', background: '#38bdf8', color: '#000', padding: '1.2rem', borderRadius: '1rem', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>أوافق والتزم بالميثاق</button>
            </div>
          );
        }
        return (
          <div style={{ animation: 'fadeIn 0.5s' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ color: '#38bdf8' }}>غرفة تداول النخبة (محاكاة تدريبية)</h3>
            ...
            <div style={styles.chatBox}>
              {chatMessages.map(m => (
                <div key={m.id} style={{ alignSelf: m.sender === 'bot' ? 'center' : 'flex-end', background: m.sender === 'bot' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(56, 189, 248, 0.1)', padding: '1rem', borderRadius: '1rem', border: m.sender === 'bot' ? '1px solid #ef4444' : '1px solid #38bdf8', maxWidth: '85%' }}>
                  {m.sender === 'member' && <small style={{ color: '#38bdf8', display: 'block', marginBottom: '5px' }}>{m.memberName} | <span style={{fontSize: '0.7rem', opacity: 0.8}}>{m.rank}</span></small>}
            ...
            {contractStep === 1 && (
              <div style={{ background: 'rgba(255,255,255,0.02)', padding: '2rem', borderRadius: '1.5rem', border: '1px solid #fbbf2444' }}>
                <div style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem' }}>
                  ...
                  <button onClick={() => setContractType('URFI')} style={{ flex: 1, padding: '1rem', borderRadius: '0.8rem', border: 'none', background: contractType === 'URFI' ? '#fbbf24' : '#333', color: contractType === 'URFI' ? '#000' : '#fff', cursor: 'pointer', fontWeight: 'bold' }}>عرفية</button>
                </div>
            ...
            {contractStep === 3 && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ background: '#fff', color: '#1a1a1a', padding: '2rem', borderRadius: '0.5rem', marginBottom: '2rem', textAlign: 'right' }}>
                  ...
                  {contractType === 'URFI' && <p style={{ color: 'red' }}>تنبيه تعليمي: يستوجب التصديق في البلدية لمنحه حجية التاريخ الثابت</p>}
                </div>
      ...
      case 'الإجراءات القانونية':
        return (
          <div style={{ animation: 'fadeIn 0.5s' }}>
            <h2 style={{ color: '#10b981', textAlign: 'center', marginBottom: '2rem' }}>حاسبة أجال الطعون (تخطي الجمعة والسبت)</h2>
            <div style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '2.5rem', borderRadius: '2rem', border: '1px solid #10b98155' }}>
              ...
              {deadlineResult && (
                <div style={{ marginTop: '2rem', padding: '1.5rem', background: 'rgba(0,0,0,0.4)', borderRadius: '1rem', textAlign: 'center', border: '1px dashed #10b981' }}>
                  ...
                  <div style={{marginTop: '10px', fontSize: '0.8rem', opacity: 0.7}}>* تم مراعاة أيام العطل الأسبوعية (الجمعة والسبت) طبقاً للتشريع المعمول به.</div>
                </div>
              )}
            </div>
          </div>
      ...
      case 'الرادار القانوني':
        return (
          <div style={{ animation: 'fadeIn 0.5s', textAlign: 'center' }}>
            ...
            <div style={{ padding: '3rem', background: 'rgba(56, 189, 248, 0.05)', borderRadius: '2rem', border: '1px solid rgba(56, 189, 248, 0.2)' }}>
              <div style={{marginBottom: '2rem', textAlign: 'right'}}>
                <div style={{background: 'rgba(56,189,248,0.1)', padding: '1rem', borderRadius: '0.5rem', marginBottom: '1rem'}}>
                  <strong>آخر تحديث:</strong> تم مطابقة البيانات مع الجريدة الرسمية الصادرة بتاريخ اليوم 2026-01-23.
                </div>
                <ul style={{lineHeight: '2'}}>
                  <li>متابعة نصوص قانون العمل الجديد.</li>
                  <li>رصد تعديلات قانون الإجراءات المدنية.</li>
                  <li>تنبيهات القوانين الصادرة في مطلع 2026.</li>
                </ul>
              </div>
              <p>... جاري فحص المستجدات التشريعية لعام 2026 ...</p>
            </div>
          </div>
        );
      case 'البحث العلمي':
        return (
          <div style={{ animation: 'fadeIn 0.5s' }}>
            ...
            <textarea placeholder="أدخل إشكالية البحث هنا لتوليد هيكل مذكرة بحثية تدريبية ..." style={{ ...styles.inputStyle, minHeight: '200px' }} />
            <button style={{ width: '100%', background: '#f87171', color: '#fff', fontWeight: 'bold', padding: '1.2rem', borderRadius: '1rem', border: 'none', marginTop: '1rem' }}>توليد مسودة البحث بصيغة Word</button>
          </div>
        );
      default:
        return (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
            {[
              ...
              <div key={i} onClick={() => setCurrentSection(s.t as Section)} style={{ background: 'rgba(255, 255, 255, 0.04)', borderRadius: '1.5rem', padding: '2.5rem', border: '1px solid rgba(255, 255, 255, 0.08)', cursor: 'pointer', textAlign: 'center' }}>
    ...
    <div style={styles.container}>
      ...
      <div style={styles.mainGlassCard}>
        <nav style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3rem', alignItems: 'center' }}>
          ...
          {currentSection !== 'main' && <button onClick={() => setCurrentSection('main')} style={{ background: 'transparent', border: '1px solid #d97706', color: '#fff', padding: '0.6rem 1.2rem', borderRadius: '0.8rem', cursor: 'pointer' }}>الرئيسية</button>}
        </nav>
        {renderContent()}
        <footer style={{ marginTop: '4rem', textAlign: 'center', opacity: 0.5 }}>
          <p>جميع الحقوق محفوظة 2026 - نظام التدريب القانوني</p>
        </footer>
      </div>
