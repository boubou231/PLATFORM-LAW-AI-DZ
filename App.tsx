  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        text: "جاري تحليل مدخلاتكم بناءً على التشريعات الصادرة في الجريدة الرسمية لعام 2026. يرجى الانتظار...",
        sender: 'ai'
      }]);
    }, 1000);
  };
  ...
  const renderContent = () => {
    switch (currentSection) {
          ...
          <div style={{ animation: 'fadeIn 0.5s' }}>
            ...
            <div style={styles.chatBox}>
              <div style={styles.bubbleAi}>
                <p style={{ margin: 0 }}>مرحباً بك في وحدة تحليل المستندات السيادية. يمكنك رفع عدة صور لعقد أو عريضة أو ملف PDF لتحليل ثغراتها القانونية بناءً على قوانين 2026.</p>
              </div>
                   ...
                   {msg.images && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                      {msg.images.map((img, idx) => (
                        <img key={idx} src={img} alt="doc" style={{ height: '80px', borderRadius: '0.5rem' }} />
                      ))}
                    </div>
                  )}
      ...
      case 'المصادر والمراجع':
        return (
          <div style={{ animation: 'fadeIn 0.5s' }}>
            <h2 style={{ color: '#a78bfa', marginBottom: '2rem', textAlign: 'center' }}>المستودع السيادي للمراجع والمؤسسات 🏛️</h2>
            <div style={{ 
                display: 'grid', 
              ...
              {[
                { n: "الأمانة العامة للحكومة - الجريدة الرسمية (JORADP)", t: "التشريعات والقرارات" },
                { n: "رئاسة الجمهورية الجزائرية", t: "القرارات السيادية" },
                { n: "المحكمة الدستورية", t: "الرقابة الدستورية" },
                { n: "المجلس الأعلى للقضاء", t: "شؤون العدالة" },
                { n: "مجلس الدولة", t: "القضاء الإداري" },
                { n: "المحكمة العليا", t: "أعلى سلطة قضائية" },
                { n: "مجلس المحاسبة", t: "الرقابة المالية" },
                { n: "وزارة العدل - البوابة القانونية", t: "الخدمات القانونية" },
                { n: "بوابة المجلات العلمية (ASJP)", t: "البحوث الأكاديمية" },
                { n: "المكتبة الوطنية الجزائرية", t: "الأرشيف والمخطوطات" },
                { n: "المجلس الإسلامي الأعلى", t: "الفتاوى والاجتهاد" },
                { n: "المجلس الوطني لحقوق الإنسان", t: "الحقوق والحريات" },
                { n: "الاتحاد الوطني لمنظمات المحامين", t: "المهن القانونية" },
                { n: "الغرفة الوطنية للموثقين", t: "التوثيق الرسمي" },
                { n: "الغرفة الوطنية للمحضرين القضائيين", t: "التنفيذ القضائي" },
                { n: "السلطة الوطنية المستقلة للانتخابات", t: "المسار الانتخابي" },
                { n: "الهيئة العليا للوقاية من الفساد ومكافحته", t: "الشفافية" },
                { n: "المعهد الجزائري للملكية الصناعية (INAPI)", t: "الملكية الفكرية" }
              ].map((m, i) => (
                <div key={i} style={{ padding: '1.2rem', background: 'rgba(255,255,255,0.03)', borderRadius: '1rem', border: '1px solid rgba(167, 139, 250, 0.2)' }}>
                  <div style={{ color: '#a78bfa', fontSize: '0.75rem', marginBottom: '0.4rem' }}>{m.t}</div>
                  <div style={{ fontWeight: 'bold' }}>🏛️ {m.n}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={styles.mainGlassCard}>
        ...
        <footer style={{ marginTop: '4rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
           <div style={{ background: 'rgba(20, 184, 166, 0.1)', padding: '1.5rem', borderRadius: '1.5rem', border: '1px solid #2dd4bf33' }}>
              ...
              <p style={{ fontSize: '0.8',
                         color: '#94a3b8' }}>تشفير سيادي كامل لعام 2026.</p>
           </div>
